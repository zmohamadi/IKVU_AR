...
export function Form({ laraPath, id, course, nextPath = "" }) {
    ...
    useEffect(() => {
        if (component?.state?.info) {
            ...
            setTimeout(() => {
                if(component?.state?.info?.for_students) handleStudentSelection("", 2);
                else handleStudentSelection();
            }, 1000);
        }
    ...
    const handleStudentSelection = function (e = "", forAll = 1, parent = component) {
        let info = parent?.state?.refs?.current;
        let semester = info?.semester?.value;
        let groupCode = info?.group_code?.value;
        // let valueSelected = (e == "")? selectFor : e.target.value;
                ...
                <div className="col-span-12 pos intro-y grid grid-cols-12 gap-5 mt-5">
                    <div className="intro-y col-span-12 lg:col-span-8">
                        ...
                        <div className="post intro-y p-3 box mt-5 ">
                            <div className="lg:grid grid-cols-12 gap-5 mt-5">
                                <YearGroup component={component} needles={needles} onGroupChange = {function(){handleStudentSelection()}} />
                            </div>
                    ...
                    <div className="col-span-12 lg:col-span-4">
                        <div className="intro-y box p-5">
                            ...
                            <CheckBox className="lg:col-span-8 mt-3" label="access_to_students" name={Lang('public.view_score_answers')}
                                refItem={[component, "to_students"]} defaultValue={component?.state?.info?.to_students} />
                            <CheckBox className="lg:col-span-4 mt-3" label="status" name={Lang('public.active')} refItem={[component, "status_id"]}
