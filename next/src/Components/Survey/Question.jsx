...
const OptionInput = ({ index, optionIndex, option, parent, questionIndex }) => (
    <div className="col-span-12 grid grid-cols-12">
        <Input
            ...
            refItem={[parent, `qOption_${questionIndex}#${option}`]}
            defaultValue={option?.title}
            className="col-span-10"
        />
        <Input
            label={`order`}
            refItem={[parent, `orderOption_${questionIndex}#${option}`]}
...
export function Question({ index, parent, addIcon, closeIcon }) {
    ...
    const options = Array.from({ length: 4 }, (_, i) => ({
        id:  question?.question_options?.[i]?.id ? `id${question?.question_options?.[i]?.id}` : i + 1,
        ...
        <Box shadow={false}>
            ...
            {options.map((option, optionIndex) => (
                <OptionInput
                    ...
                    option={option.id}
                    parent={parent}
                    questionIndex={option.question_id}
                />
            ))}
