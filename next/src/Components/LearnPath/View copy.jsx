        ...
        <div className="mx-auto">
            ...
            <div>
                ...
                <div className="rounded-lg dark:bg-gray-800" id="image_pdf" tab-group="tabpanel">
                    {
                        data.image_pdf?.map((item) => {
                            let ext = item?.substr(item?.length - 4);

                            if (ext == '.bcr') {
                                return <div className="intro-y box px-2 md:px-5 mt-5" style={{ background: '#1e2434' }} id="reader">
                                    <div className={isMobile ? "px-0 md:px-5 py-5 flex min-h-[80vh] relative items-start justify-center pdf-reader" : "px-0 md:px-5 py-5 flex min-h-[90vh] relative items-start justify-center pdf-reader"}>
                                        <Pdf fileUrl={fileUrl} file={item} />
                                    </div>
                                </div>
                            } else {
                                images.push(item);
